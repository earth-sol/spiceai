...
Spice v1.0.4 enhances partition pruning for Delta Lake tables with `time_partition_column` and `time_partition_format` parameters, fixes support for uppercase columns when used in refresh SQL, indexes, or primary keys, makes Grok tool calling more robust, and enhances the 'spice trace' tool.

## Highlights in v1.0.4

- **Partition pruning for Delta Lake**: Use `time_partition_column` and `time_partition_format` to eliminate unnecessary partitions and reduce the amount of data loaded and processed.

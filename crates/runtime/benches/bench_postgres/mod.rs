...
pub(crate) fn get_postgres_params(is_acc: bool, bench_name: &str) -> Params {
    ...
    let pg_port = std::env::var("PG_BENCHMARK_PG_PORT").unwrap_or_default();
...
pub(crate) fn get_tpch_test_queries() -> Vec<(&'static str, &'static str)> {
    vec![
...
#[allow(clippy::too_many_lines)]
pub(crate) fn get_tpcds_test_queries() -> Vec<(&'static str, &'static str)> {
    vec![

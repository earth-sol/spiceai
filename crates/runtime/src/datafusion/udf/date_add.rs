...
use arrow::array::{
    Date64Array, Int64Array, IntervalMonthDayNanoArray, StringArray, Time64NanosecondArray,
...
fn add_months_to_timestamp_nanos(timestamp_nanos: i64, months: i64) -> Option<i64> {
...
// Helper function to add months to a Date64 (days since epoch)
fn add_months_to_date64(date_days: i64, months: i64) -> Option<i64> {
    let timestamp_nanos = date_days * NANOSECONDS_IN_DAY;
...
impl DateAddFunc {
    pub fn new() -> Self {
        let signature = Signature::one_of(
            vec![
                // DATE_ADD(string, int32) -> Date64
                Exact(vec![DataType::Utf8, DataType::Int64]),
                // DATE_ADD(Date64, int32) -> Date64
                ...
                // DATE_ADD(Date64, IntervalMonthDayNano) -> TimestampMillisecond
                Exact(vec![
                    DataType::Date64,
                    DataType::Interval(IntervalUnit::MonthDayNano),
                ]),
...
impl ScalarUDFImpl for DateAddFunc {
    ...
    fn return_type(&self, arg_types: &[DataType]) -> Result<DataType> {
        match (&arg_types[0], &arg_types[1]) {
            (DataType::Utf8, DataType::Int64) => Ok(DataType::Date64),
            (DataType::Date64, DataType::Int64) => Ok(DataType::Date64),
            (DataType::Utf8, DataType::Interval(IntervalUnit::MonthDayNano)) => {
                Ok(DataType::Timestamp(TimeUnit::Millisecond, None))
            }
            (DataType::Date64, DataType::Interval(IntervalUnit::MonthDayNano)) => {
                Ok(DataType::Timestamp(TimeUnit::Millisecond, None))
            }
            (
...
fn date_add_impl(value: &ColumnarValue, interval: &ColumnarValue) -> Result<ColumnarValue> {
    match (value, interval) {
        // DATE_ADD(string, int32) -> Date64
        (ColumnarValue::Array(arr1), ColumnarValue::Array(arr2))
            if arr1.data_type() == &DataType::Utf8 && arr2.data_type() == &DataType::Int64 =>
        {
            let strings = arr1.as_any().downcast_ref::<StringArray>().unwrap();
            let days = arr2.as_any().downcast_ref::<Int64Array>().unwrap();
            let result: Date64Array = strings
                .iter()
                .zip(days.iter())
                .map(|(s, days)| {
                    s.and_then(|s| {
                        days.and_then(|n| {
                            ...
                            let date_days = nanos / NANOSECONDS_IN_DAY;
                            Some(date_days + n)
                        })
                    })
                })
        ...
        // DATE_ADD(Date64, int32) -> Date64
        (ColumnarValue::Array(arr1), ColumnarValue::Array(arr2))
            if arr1.data_type() == &DataType::Date64 && arr2.data_type() == &DataType::Int64 =>
        {
            let dates = arr1.as_any().downcast_ref::<Date64Array>().unwrap();
            let days = arr2.as_any().downcast_ref::<Int64Array>().unwrap();
            let result: Date64Array = dates
                .iter()
                .zip(days.iter())
        ...
        (ColumnarValue::Array(arr1), ColumnarValue::Array(arr2))
            if arr1.data_type() == &DataType::Utf8
                && arr2.data_type() == &DataType::Interval(IntervalUnit::MonthDayNano) =>
        {
            let strings = arr1.as_any().downcast_ref::<StringArray>().unwrap();
            let intervals = arr2
                .as_any()
                    ...
                    s.and_then(|s| {
                        interval.and_then(|i| {
                            ...
                            let timestamp_ms = (base_timestamp_nanos / NANOSECONDS_IN_DAY)
                                * NANOSECONDS_IN_DAY
                                / NANOS_PER_MILLI;
                            let interval_ms = (interval_days as i64 * MILLISECONDS_IN_DAY)
        ...
        // DATE_ADD(Date64, IntervalMonthDayNano) -> TimestampMillisecond
        (ColumnarValue::Array(arr1), ColumnarValue::Array(arr2))
            if arr1.data_type() == &DataType::Date64
                && arr2.data_type() == &DataType::Interval(IntervalUnit::MonthDayNano) =>
        {
            let dates = arr1.as_any().downcast_ref::<Date64Array>().unwrap();
            let intervals = arr2
                    ...
                    date.and_then(|d| {
                        interval.and_then(|i| {
                            ...
                            let base_timestamp_ms = if months != 0 {
                                add_months_to_date64(d, months as _)? / NANOS_PER_MILLI
                            } else {
                    ...
                    ts.and_then(|t| {
                        interval.and_then(|i| {
                            ...
                            let base_timestamp_ms = if months != 0 {
                                ...
                                add_months_to_timestamp_nanos(timestamp_nanos, months as _)?
                                    / NANOS_PER_MILLI
                            } else {
                                t
                            };
                ...
                .map(|(time, interval)| {
                    time.and_then(|t| {
                        interval.and_then(|i| {
                            ...
                            let total_nanos = t + (days as i64 * NANOSECONDS_IN_DAY) + nanos;
        ...
        (ColumnarValue::Scalar(scalar1), ColumnarValue::Scalar(scalar2)) => {
            match (scalar1, scalar2) {
                (ScalarValue::Utf8(Some(s)), ScalarValue::Int64(Some(days))) => {
                    let nanos = string_to_timestamp_nanos(s).map_err(|e| {
                        DataFusionError::Execution(format!("Failed to parse timestamp: {}", e))
                    })?;
                    let date_days = nanos / NANOSECONDS_IN_DAY;
                    Ok(ColumnarValue::Scalar(ScalarValue::Date64(Some(
                        date_days + days,
                    ))))
                }
                (ScalarValue::Date64(Some(date)), ScalarValue::Int64(Some(days))) => Ok(
                    ColumnarValue::Scalar(ScalarValue::Date64(Some(date + days))),
                ),
                (ScalarValue::Utf8(Some(s)), ScalarValue::IntervalMonthDayNano(Some(interval))) => {
                    let nanos = string_to_timestamp_nanos(s).map_err(|e| {
                        DataFusionError::Execution(format!("Failed to parse timestamp: {}", e))
                    })?;
                    ...
                    let base_timestamp_nanos = if months != 0 {
                        add_months_to_timestamp_nanos(nanos, months as _).ok_or_else(|| {
                    ...
                    let timestamp_ms = (base_timestamp_nanos / NANOSECONDS_IN_DAY)
                        * NANOSECONDS_IN_DAY
                        / NANOS_PER_MILLI;
                    let interval_ms =
                        (days as i64 * MILLISECONDS_IN_DAY) + (interval_nanos / NANOS_PER_MILLI);
                ...
                (
                    ...
                    let base_timestamp_ms = if months != 0 {
                        add_months_to_date64(*date, months as _).ok_or_else(|| {
                            DataFusionError::Execution("Failed to add months to date".to_string())
                        })? / NANOS_PER_MILLI
                    } else {
                ...
                (
                    ...
                    let base_timestamp_ms = if months != 0 {
                        ...
                        add_months_to_timestamp_nanos(timestamp_nanos, months as _).ok_or_else(
                            || {
                                DataFusionError::Execution(
                                    "Failed to add months to timestamp".to_string(),
                                )
                ...
                (
                    ...
                    let (months, days, nanos) = IntervalMonthDayNanoType::to_parts(*interval);
                    if months != 0 {
                        return exec_err!("Month intervals not supported for time in date_add");
                    }
...
mod tests {
    ...
    use arrow::array::{
        ArrayRef, Date64Array, Int64Array, IntervalMonthDayNanoArray, StringArray,
    ...
    fn make_interval_month_day_nano(months: i32, days: i32, nanos: i64) -> IntervalMonthDayNano {
    ...
    async fn test_basic() -> Result<(), Box<dyn std::error::Error + Send + Sync>> {
        ...
        let sql = "SELECT date_add('2022-01-01', 2)";
        let actual = ctx.sql(sql).await?.collect().await?;

        assert_batches_eq!(&[""], &actual);
        Ok(())
    }
    ...
    fn test_date_add_string_2_days() {
        ...
        run_test_case(
            ...
            DataType::Date64,
            Arc::new(Date64Array::from(vec![Some(18995)])) as ArrayRef, // 2022-01-03
        );
    }
    ...
    fn test_date_add_string_minus_2_days() {
        ...
        run_test_case(
            ...
            ColumnarValue::Array(Arc::new(Int64Array::from(vec![-2]))),
            DataType::Date64,
            Arc::new(Date64Array::from(vec![Some(18991)])) as ArrayRef, // 2021-12-30
        );
    }
    ...
    fn test_date_add_date_30_days() {
        ...
        run_test_case(
            ColumnarValue::Array(Arc::new(Date64Array::from(vec![Some(18993)]))), // 2022-01-01
            ColumnarValue::Array(Arc::new(Int64Array::from(vec![30]))),
            DataType::Date64,
            Arc::new(Date64Array::from(vec![Some(19023)])) as ArrayRef, // 2022-01-31
        );
    }
    ...
    fn test_date_add_date_minus_30_days() {
        ...
        run_test_case(
            ...
            ColumnarValue::Array(Arc::new(Int64Array::from(vec![-30]))),
            DataType::Date64,
            Arc::new(Date64Array::from(vec![Some(18963)])) as ArrayRef, // 2021-12-02
        );
    }
    ...
    fn test_date_add_string_2_day_interval() {
        ...
        run_test_case(
            ...
            DataType::Timestamp(TimeUnit::Millisecond, None),
            Arc::new(TimestampMillisecondArray::from(vec![Some(1641168000000)])) as ArrayRef, // 2022-01-03 00:00:00
        );
    }
    ...
    fn test_date_add_string_minus_2_day_interval() {
        ...
        run_test_case(
            ...
            DataType::Timestamp(TimeUnit::Millisecond, None),
            Arc::new(TimestampMillisecondArray::from(vec![Some(1640822400000)])) as ArrayRef, // 2021-12-30 00:00:00
        );
    }
    ...
    fn test_date_add_date_30_day_interval() {
        ...
        run_test_case(
            ...
            DataType::Timestamp(TimeUnit::Millisecond, None),
            Arc::new(TimestampMillisecondArray::from(vec![Some(1643587200000)])) as ArrayRef, // 2022-01-31 00:00:00
        );
    }
    ...
    fn test_date_add_timestamp_string_30_day_interval() {
        ...
        run_test_case(
            ColumnarValue::Array(Arc::new(StringArray::from(vec!["2022-01-01 12:00:00"]))),
            ColumnarValue::Array(Arc::new(IntervalMonthDayNanoArray::from(vec![Some(
                make_interval_month_day_nano(0, 30, 0),
            )]))),
            DataType::Timestamp(TimeUnit::Millisecond, None),
    ...
    fn test_date_add_timestamp_string_minus_30_day_interval() {
        ...
        run_test_case(
            ...
            DataType::Timestamp(TimeUnit::Millisecond, None),
            Arc::new(TimestampMillisecondArray::from(vec![Some(1638403200000)])) as ArrayRef, // 2021-12-02 00:00:00
        );
    }
    ...
    fn test_date_add_timestamp_30_day_interval() {
        ...
        run_test_case(
            ...
            DataType::Timestamp(TimeUnit::Millisecond, None),
            Arc::new(TimestampMillisecondArray::from(vec![Some(1643630400000)])) as ArrayRef, // 2022-01-31 12:00:00
        );
    }
    ...
    fn test_date_add_timestamp_minus_30_day_interval() {
        ...
        run_test_case(
            ...
            DataType::Timestamp(TimeUnit::Millisecond, None),
            Arc::new(TimestampMillisecondArray::from(vec![Some(1638446400000)])) as ArrayRef, // 2021-12-02 12:00:00
        );
    }
    ...
    fn test_date_add_string_1_month_interval() {
        ...
        run_test_case(
            ...
            DataType::Timestamp(TimeUnit::Millisecond, None),
            Arc::new(TimestampMillisecondArray::from(vec![Some(1643673600000)])) as ArrayRef, // 2022-02-01 00:00:00
        );
    }
    ...
    fn test_date_add_string_minus_1_month_interval() {
        ...
        run_test_case(
            ...
            DataType::Timestamp(TimeUnit::Millisecond, None),
            Arc::new(TimestampMillisecondArray::from(vec![Some(1640995200000)])) as ArrayRef, // 2022-01-01 00:00:00
        );
    }
    ...
    fn test_date_add_date_1_month_interval() {
        ...
        run_test_case(
            ColumnarValue::Array(Arc::new(Date64Array::from(vec![Some(19023)]))), // 2022-01-31
            ColumnarValue::Array(Arc::new(IntervalMonthDayNanoArray::from(vec![Some(
                make_interval_month_day_nano(1, 0, 0),
            )]))),
            DataType::Timestamp(TimeUnit::Millisecond, None),
            Arc::new(TimestampMillisecondArray::from(vec![Some(1646006400000)])) as ArrayRef, // 2022-02-28 00:00:00
        );
    }
    ...
    fn test_date_add_date_minus_1_month_interval() {
        ...
        run_test_case(
            ColumnarValue::Array(Arc::new(Date64Array::from(vec![Some(19051)]))), // 2022-02-28
            ColumnarValue::Array(Arc::new(IntervalMonthDayNanoArray::from(vec![Some(
                make_interval_month_day_nano(-1, 0, 0),
            )]))),
            DataType::Timestamp(TimeUnit::Millisecond, None),
            Arc::new(TimestampMillisecondArray::from(vec![Some(1643328000000)])) as ArrayRef, // 2022-01-28 00:00:00
        );
    }
    ...
    fn test_date_add_timestamp_1_month_interval() {
        ...
        run_test_case(
            ...
            DataType::Timestamp(TimeUnit::Millisecond, None),
            Arc::new(TimestampMillisecondArray::from(vec![Some(1646049600000)])) as ArrayRef, // 2022-02-28 12:00:00
        );
    }
    ...
    fn test_date_add_timestamp_minus_1_month_interval() {
        ...
        run_test_case(
            ...
            DataType::Timestamp(TimeUnit::Millisecond, None),
            Arc::new(TimestampMillisecondArray::from(vec![Some(1643371200000)])) as ArrayRef, // 2022-01-28 12:00:00
        );
    }
    ...
    fn test_date_add_string_1_month_1_day_interval() {
        ...
        run_test_case(
            ...
            DataType::Timestamp(TimeUnit::Millisecond, None),
            Arc::new(TimestampMillisecondArray::from(vec![Some(1646092800000)])) as ArrayRef, // 2022-03-01 00:00:00
        );
    }

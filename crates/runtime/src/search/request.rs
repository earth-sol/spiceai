...
use datafusion::sql::sqlparser::ast::{Expr, SelectItem, TableFactor, TableWithJoins, Value, ValueWithSpan};
...
impl SearchRequest {
    ...
    pub fn validate_keyword_to_ilike(k: &str, target_column: &str) -> Result<Expr> {
        ...
        if let (
            Expr::Identifier(id),
            Expr::Value(ValueWithSpan { value: Value::SingleQuotedString(v), .. }),
